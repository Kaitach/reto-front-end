...
async mapAccountData(decodedToken: DecodedToken): Promise<UserEntity> {
  ...
  async login(data: { email: string, password: string }) {
    ...
    const response = await this.factory.userLogin.useFactory(this.userRepository).execute(data).toPromise();
  ...
  async register(account: UserModel){
    this.factory.createUser.useFactory(this.userRepository).execute(account).subscribe(
      (data) => {
