...
export class AccountComponent implements OnInit {
  ...
  getUserAccounts(): void {
    this.user.Account.map((accountId) => {
      return this.factoryAccount.getAccountById
        .useFactory(this.accountRepository)
        .execute(accountId)
        .pipe(
          map((account) => {
  ...
  async deleteAccount(id: string) {
    ...
    if ((await deleteAccount) == true) {
      this.factoryAccount.deleteAccount
        .useFactory(this.accountRepository)
        .execute(id)
        .subscribe();
  ...
  updateAccount(): void {
    ...
    this.factoryAccount.UpdateAccount.useFactory(this.accountRepository)
      .execute(this.id, account)
      .subscribe();
  ...
  openModal(id: string, amount: number): void {
